...
const updateImpostor = (): void => {
  ...
  try {
    ...
    console.warn('Failed to create octahedral material, using fallback:', error)
  ...
  console.log('✅ Basic material created with WebGPU + TSL (octahedral logic coming next)')
...
const setupEventListeners = (): void => {
  ...
  // File inputs
  const albedoInput = document.getElementById('albedo-input') as HTMLInputElement
  ...
  // Controls
  const alphaSlider = document.getElementById('alpha-clamp') as HTMLInputElement
  const alphaValue = document.getElementById('alpha-value') as HTMLSpanElement
  const blendingCheckbox = document.getElementById('disable-blending') as HTMLInputElement
  const hemisphereCheckbox = document.getElementById('hemispherical-mode') as HTMLInputElement
  const restrictCameraCheckbox = document.getElementById('restrict-camera') as HTMLInputElement
  const elevationSlider = document.getElementById('elevation-threshold') as HTMLInputElement
  ...
  restrictCameraCheckbox?.addEventListener('change', event => {
    ...
    console.log(`Camera restriction below Y=0: ${config.restrictCameraBelow ? 'ON' : 'OFF'}`)
  })
